...
def filtrage_amp(data, A1, A2): # Soient A1 et A2 les amplitudes des bornes
    ...
    spectre_filtre = np.zeros(len(spectre),dtype=complex)    # On crée un array vide qui va accueillir le spectre filtré
    for i in range(len(spectre)):
        #print(np.abs(spectre[i]))
        if (np.abs(spectre[i]) < A1 or np.abs(spectre[i]) > A2):
            spectre_filtre[i]=0
        else:
            spectre_filtre[i]=spectre[i]
